...
class CrossAttentionSepHead(nn.Module):
    def __init__(
        self, dim, num_heads=8, qkv_bias=False
    ):
        super().__init__()
    ...
    def forward(self, x, y, mask=None):
        ...
        B1, H1, N1, C = x.shape # bs, heads, roles, dimension
        B2, H2, N2, C = y.shape # bs, heads, image_tokens, dimension
        assert B1 == B2, "role and image tokens should have same batch size"
        q = self.q(x).reshape(
            B1, N1, 1, H1, C
        ).permute(2, 0, 3, 1, 4)
...
class CrossAttentionSepHeadBlock(nn.Module):
    def __init__(
        self,
        dim,
        num_heads=None,
        ...
        self.attn = CrossAttentionSepHead(
            dim,
            num_heads=num_heads,
            qkv_bias=qkv_bias,
        )
    ...
    def forward(self, x, y, mask=None):
        ...
        x_block = x_block.squeeze(0).permute(1,0,2,3)
...
class SelfAttentionSepHead(nn.Module):
    def __init__(
        self, dim, num_heads=8, qkv_bias=False, proj_drop=0.
    ):
        super().__init__()
...
class SelfAttentionSepHeadBlock(nn.Module):
    def __init__(
        self,
        dim,
        num_heads=None,
        ...
        self.dim = dim
        self.norm1 = norm_layer(dim)
        self.attn = SelfAttentionSepHead(
            dim,
            num_heads=num_heads,
            qkv_bias=qkv_bias,
            proj_drop=drop_rate,
