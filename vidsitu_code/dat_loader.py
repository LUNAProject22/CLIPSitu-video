...
class VsituDS(Dataset):
    ...
    def set_comm_args(self):
        ...
        elif self.sf_cfg.MODEL.ARCH == 'mlp':
            self.comm.path_type = "mlp"
        else:
            raise NotImplementedError
    ...
    def read_files(self, split_type: str):
        ...
        #         try:
        #             data = pickle.load(f)
        #             clip_feat_files_new.append(file)
        #         except:
        #             continue
    ...
    def vb_only_item_getter(self, idx: int):
        ...
        clip_frms_out_dct = self.get_clip_frm_feats_all(idx)
        clip_frms_out_dct["vseg_idx"] = torch.tensor(idx)
        label_out_dct = self.get_label_out_dct(idx)
        out_dct = coalesce_dicts([clip_frms_out_dct, label_out_dct])
        return out_dct
    ...
    def vb_args_item_getter(self, idx: int):
        ...
        if self.full_cfg.mdl.mdl_name not in set(
            ...
            frm_feats_out_dct = self.get_clip_frm_feats_all(idx)
